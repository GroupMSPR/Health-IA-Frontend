...
export default function FoodScanIAResultPage() {
    ...
    // On récupère le vrai résultat de l'IA ET l'image scannée
    const food = location.state?.aiResult as FoodAIResult;
    const scannedImageUrl = location.state?.scannedImageUrl as string | null;
                ...
                mutate: [
                    {
                        ...
                        attributes: {
                            name: food.name,
                            category: food.category || 'All', // Sécurité
                            image: food.image,
                            ...
                            <div className="relative z-10 p-6 sm:p-8 w-full">
                                <div className="flex flex-wrap gap-2 items-center mb-2">
                                    <span className={`px-3 py-1 rounded-md text-xs font-bold shadow-sm ${getCategoryStyles(food.category)}`}>
                                        {filterTranslations[food.category] || food.category || 'Aliment'}
                                    </span>
                                ...
                                <div className="mt-8 pt-6 border-t border-slate-100 space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="flex justify-between text-sm p-3 bg-slate-50 rounded-xl">
                                            ...
                                            <span className="font-bold text-slate-700">{Number(food.sugars) || 0}g</span>
                                        </div>
                                        <div className="flex justify-between text-sm p-3 bg-slate-50 rounded-xl">
                                            <span className="font-medium text-slate-500">Fibres alimentaires</span>
                                            <span className="font-bold text-slate-700">{Number(food.fiber) || 0}g</span>
                                        </div>
                                        <div className="flex justify-between text-sm p-3 bg-slate-50 rounded-xl">
                                            <span className="font-medium text-slate-500">Sodium</span>
                                            <span className="font-bold text-slate-700">{Number(food.sodium) || 0}mg</span>
                                        </div>
                                        <div className="flex justify-between text-sm p-3 bg-slate-50 rounded-xl">
                                            <span className="font-medium text-slate-500">Cholestérol</span>
                                            <span className="font-bold text-slate-700">{Number(food.cholesterol) || 0}mg</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
